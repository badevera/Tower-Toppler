...
int BuildSpriteShader(bool collisionTest = false) {
	const char *vShader = R"(
		#version 410 // 410 for Mac, else 330
		uniform mat4 view;
		uniform float z = 0;
		out vec2 uv;
		void main() {
...
void Sprite::Initialize(string imageFile, float z, bool compensateAspectRatio) {
	...
	if (ReadTexture(imageFile.c_str(), &textureName, true, &nTexChannels, &imgWidth, &imgHeight)) 
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);
	UpdateTransform();
}
...
void Sprite::UpdateTransform() {
	mat4 yRot = Translate(position.x, position.y, z)*RotateY(yRotation)*Translate(0, 0, -z);
	ptTransform = Translate(position.x, position.y, 0)*RotateZ(rotation)*Scale(scale.x, scale.y, 1)*yRot;
...
void Sprite::SetUVforFrame(int row, int col, int nRows, int nCols) {
	float fRows = (float) nRows, fCols = (float) nCols, fRow = (float) row, fCol = (float) col;
	mat4 t = Translate(fCol/fCols, 1-(fRow+1)/fRows, 0);
	mat4 s = Scale(1.f/fCols, 1.f/fRows, 1);
	SetUvTransform(t*s);
}
