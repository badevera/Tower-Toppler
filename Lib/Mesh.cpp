...
void Mesh::ApplyTransform(mat4 &m) {
	toWorld = m*toWorld;
	for (size_t i = 0; i < children.size(); i++)
		children[i]->ApplyTransform(m);
}
...
void Mesh::SetWrtParent() {
	...
	if (parent != NULL) {
		mat4 inv = Inverse(parent->toWorld);
...
// simplest scheme is to apply transform to mesh and all its descendents
// see 7-Assn-Hierarchy-Solution: ApplyTransform (simplest scheme)
// see 22-Ariculate.cpp: uses SetWrtParent
...
bool Mesh::Read(string objFile, string texFile, mat4 *m, bool standardize, bool buffer, bool forceTriangles) {
	...
	objFilename = objFile;
	texFilename = texFile;
	if (!ReadTexture((char *) texFile.c_str(), &textureName))
