...
// mouse operations (default viewport int4(0,0,0,0) will use current vp)
bool MouseOver(double xmouse, double ymouse, vec3 p, mat4 &view, int proximity = 16, int4 vp = int4(0, 0, 0, 0));
bool MouseOver(double xmouse, double ymouse, vec2 p, int proximity = 16);
bool MouseOver(int xmouse, int ymouse, vec2 p, int proximity = 16);
	// is mouse(x,y) within proximity pixels of screen point p?
vec3 *MouseOver(vec3 *points, int npoints, int xmouse, int ymouse, mat4 fullview, int proximity = 16, int4 vp = int4(0, 0, 0, 0));
vec3 *MouseOver(vector<vec3> &points, int xmouse, int ymouse, mat4 fullview, int proximity = 16, int4 vp = int4(0, 0, 0, 0));
...
public:
	...
	mat4 Drag(int xmouse, int ymouse);
		// update *m if non-null, return matxix to apply to children
	void Up();
...
public:
	...
	mat4 Drag(int x, int y, mat4 modelview, mat4 persp);
	void Up();
	void Wheel(double spin, bool shift);
...
class Joystick {
public:
	Joystick(vec3 *b = NULL, vec3 *v = NULL, float arrowScale = 1);
	bool Hit(int xmouse, int ymouse, vec3 b, vec3 v, mat4 fullview, int4 vp = int4(0,0,0,0));
	bool Hit(int xmouse, int ymouse, mat4 fullview, int4 vp = int4(0,0,0,0));
	void Down(int xmouse, int ymouse, vec3 *b, vec3 *v, mat4 modelview, mat4 persp, int4 vp = int4(0,0,0,0));
	void Down(int xmouse, int ymouse, mat4 modelview, mat4 persp, int4 vp = int4(0,0,0,0));
	void Drag(int xmouse, int ymouse, mat4 modelview, mat4 persp, int4 vp = int4(0,0,0,0));
	void Draw(mat4 modelview,mat4 persp,vec3 baseColor = vec3(0, 0, 1), vec3 lineColor = vec3(0, .6f, 0), vec3 headcolor = vec3(1, 1, 1));
	void SetVector(vec3 v);
	void SetBase(vec3 b);
	vec3 *base = NULL, *vec = NULL;		// pointers to client data
	JoyType mode = JoyType::A_None;
private:
	float arrowScale = 1, len = 1;
...
public:
	...
	Toggler(const char *name = NULL, int x = 0, int y = 0, float dia = 24,
			vec3 col = vec3(.7f), vec3 ringCol = vec3(.1f));
	Toggler(const char *name, bool on, int x, int y, float dia = 24,
			vec3 onCol = vec3(1,0,0), vec3 offCol = vec3(.7f), vec3 ringCol = vec3(.1f));
	Toggler(bool *on, const char *name, int x, int y, float dia = 24,
			vec3 onCol = vec3(1,0,0), vec3 offCol = vec3(.7f), vec3 ringCol = vec3(.1f));
	bool On();
	void Set(bool on);
	void Draw(const char *nameOverride = NULL, float textSize = 16, vec3 *color = NULL);
