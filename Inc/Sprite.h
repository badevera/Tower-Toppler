...
public:
	...
	vec2		scale = vec2(1, 1);
	float		rotation = 0;						// in degrees around z-axis
	float		yRotation = 0;
	...
	bool		enabled = true;						// else, hit-testing always fails
	...
	// display
	void Display(mat4 *view = NULL, int texUnit = 0);	// if view NULL, space presumed NDC (+/-1)
	void Outline(vec3 color, float width = 2);		// draw bounding-box
	// animation
	void SetFrame(int n);
	void SetFrameDuration(float dt);				// dt in seconds
	void SetUVforFrame(int row, int col, int nRows, int nCols);
